...
type Recipe struct {
	...
	SourceURLs  StringList `gorm:"type:jsonb;not null;default:'[]'" json:"sourceUrls"`
...
// RecipeVersion stores a full JSON snapshot of a recipe so users can move
// between AI remix versions without losing prior edits.
type RecipeVersion struct {
	UUIDModel
	InstanceID    uuid.UUID `gorm:"type:uuid;not null;index" json:"instanceId"`
	RecipeID      uuid.UUID `gorm:"type:uuid;not null;index" json:"recipeId"`
	VersionNumber int       `gorm:"not null" json:"versionNumber"`
	RemixPrompt   string    `gorm:"type:text" json:"remixPrompt"`
	Snapshot      JSONBlob  `gorm:"type:jsonb;not null" json:"snapshot"`
	CreatedByID   uuid.UUID `gorm:"type:uuid;not null" json:"createdById"`
	IsCurrent     bool      `gorm:"not null;default:false;index" json:"isCurrent"`
	CreatedAt     time.Time `json:"createdAt"`
